...
export default async function CoursesIndexPage({ searchParams }: { searchParams: Promise<{ category?: string; type?: string }> }) {
  ...
  try {
    ...
    rawSubjects = await db.collection('subjects').find({}).toArray();
    totalCourses = (await db.collection('courses').countDocuments({})) + neonCourses.length;
    publishedCourses = (await db.collection('courses').countDocuments({ status: 'published' })) + neonCourses.filter((c: any) => c.status === 'published').length;
  } catch (error) {
