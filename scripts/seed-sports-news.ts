...
async function seed() {
  console.log('--- Starting Sports Intelligence Seeding ---');
  for (const news of sportsSeeds) {
    try {
      await NewsService.upsertNews(news);
      console.log(`[OK] Seeded: ${news.title}`);
    } catch (e) {
      console.error(`[FAIL] Error seeding ${news.title}:`, e);
    }
  }
  console.log('--- Seeding Complete ---');
}

// Check if we are running this directly
