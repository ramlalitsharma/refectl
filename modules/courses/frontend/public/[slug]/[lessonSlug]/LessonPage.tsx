...
export async function generateMetadata(
  ...
  try {
    ...
    let course = await db.collection('courses').findOne({ slug }, { projection: { title: 1, units: 1, modules: 1 } });

    if (!course) {
      const neonCourse = await CourseServiceNeon.getCourseBySlug(slug);
      if (neonCourse) {
        course = { ...neonCourse, units: (neonCourse as any).curriculum || [] } as any;
      }
    }
...
export default async function LessonPage({ params }: { params: Promise<{ slug: string; lessonSlug: string }> }) {
  ...
  try {
    ...
    if (!course) {
      const neonCourse = await CourseServiceNeon.getCourseBySlug(slug);
      if (neonCourse) {
        course = { ...neonCourse, _id: (neonCourse as any).id, units: (neonCourse as any).curriculum || [] } as any;
      }
    }
    ...
    if (course) {
      ...
      for (const unit of units) {
        ...
        for (const chapter of chapters) {
          ...
          if (l) {
            lesson = l;
            courseModule = chapter; // Use chapter as the immediate parent
            break;
          }
        }
        if (lesson) break;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span>•</span>
              <span>Part of {courseModule.title}</span>
            </div>
          </div>
      ...
      <AITutor context={`Course: ${course.title}\nLesson: ${lesson.title}\nContent: ${lesson.content || 'Video/Live lesson'}`} />
    </div>
  );
}
